...
export const createTestimonial = async (req: Request, res: Response) => {
  try {
    ...
    const testimonial = await Testimonial.create({
      name,
      message,
      rating: rating ? parseInt(rating) : 5, // On s'assure que c'est un nombre
      avatar,
      isApproved: false, // Toujours faux à la création par sécurité
    });
...
export const approveTestimonial = async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string; // <--- FIX : Typage explicite
